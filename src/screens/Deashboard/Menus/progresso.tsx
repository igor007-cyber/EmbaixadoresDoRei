    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-white/40 text-sm mt-1">
          Visão geral do desempenho dos embaixadores
        </p>
      </div>
      ...
      {embaixadores.length === 0 ? (
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-8">
            {(['Arauto', 'Escudeiro', 'Sênior', 'Emérito', 'Futuro Membro'] as const).map((manual) => {
              const count = embaixadores.filter((embaixador) => embaixador.manual === manual).length
              return (
          ...
          <div className="bg-white/5 rounded-2xl border border-white/10 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {embaixadores.map((embaixador, index) => {
                    const lista = atividadesDoEmbaixador(embaixador.id)
                    const feitas = lista.filter((atividade) => atividade.feito).length
                    const total = lista.length
                    const pct = total > 0 ? Math.round((feitas / total) * 100) : 0

                    return (
                      <tr key={embaixador.id} className="border-t border-white/5 hover:bg-white/5 transition-colors">
                        <td className="p-4 text-white/20 text-xs font-mono">
                          {String(index + 1).padStart(2, '0')}
                        </td>
                        <td className="p-4 text-white font-medium">{embaixador.nome}</td>
                        <td className="p-4">
