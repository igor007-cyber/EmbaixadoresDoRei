...
export default function PerfilView({
  ...
  const campos: { label: string; key: keyof Perfil; type?: string; icon: string }[] = [
    { label: 'Usuario', key: 'username', icon: '👤' },
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">Suas informacoes de cadastro</p>
        </div>
        {!editandoPerfil ? (
          <button
            onClick={() => {
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-[#171747]! hover:bg-[#0f0f4b]! text-white font-bold rounded-xl transition shadow-lg shadow-[#F6B500]/20"
          >
            ✏️ Editar Perfil
          </button>
        ) : (
          <div className="flex gap-2">
            <button
              onClick={() => void onSave()}
              disabled={salvando}
              className="px-5 py-2.5 bg-green-500 hover:bg-green-400 disabled:opacity-60 text-white font-bold rounded-xl transition"
            >
            ...
            <button
              ...
              disabled={salvando}
              className="px-5 py-2.5 border border-white/20 text-white/60 hover:bg-white/10 disabled:opacity-60 font-bold rounded-xl transition"
            >
              Cancelar
            </button>
          </div>
            ...
            <div className="flex-1">
              ...
              {editandoPerfil ? (
                <select
                  ...
                  onChange={(e) =>
                    setPerfilTemp((p) => ({ ...p, quantidadePessoas: e.target.value }))
                  }
                  className="w-full bg-white/10 border border-white/20 rounded-lg px-3 py-1.5 text-white text-sm focus:outline-none focus:border-[#F6B500] transition"
                >
      ...
      {!perfil.username && (
        <div className="text-center py-8 text-white/20">
          ...
          <p className="text-sm">Nenhum dado encontrado. Faca o cadastro primeiro.</p>
        </div>
      )}
    </div>
  )
