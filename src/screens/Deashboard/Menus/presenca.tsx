...
interface PresencaProps {
  ...
  onAddReunioes: (dias: number[], horario: string) => Promise<string | null>
  onDeleteReuniao: (id: string) => Promise<void>
  onTogglePresenca: (reuniaoId: string, pessoaKey: string, field: 'presente' | 'versiculo') => Promise<void>
}
...
export default function Presenca({
  ...
  const todasPessoas = [
    ...embaixadores.map((embaixador) => ({
      key: `emb-${embaixador.id}`,
      nome: embaixador.nome,
      tipo: 'Embaixador' as const,
      manual: embaixador.manual,
    })),
    ...conselheiros.map((conselheiro) => ({
      key: `con-${conselheiro.id}`,
      nome: conselheiro.nome,
      tipo: 'Conselheiro' as const,
      manual: '',
    })),
  ]

  const reunioesFiltradas = reunioes
    .filter((reuniao) => reuniao.mes === mesAtual && reuniao.ano === anoAtual)
  ...
  async function handleAddReuniao() {
    ...
    const reuniaoId = await onAddReunioes(novaReuniaoDias, novaReuniaoHorario)
    setNovaReuniaoDias([])
    setShowFormReuniao(false)
    if (reuniaoId) {
      setReuniaoSelecionada(reuniaoId)
    }
  }
  ...
  function presentsCount(reuniaoId: string) {
    return todasPessoas.filter((pessoa) => registros[reuniaoId]?.[pessoa.key]?.presente).length
  }

  function versiculoCount(reuniaoId: string) {
    return todasPessoas.filter((pessoa) => registros[reuniaoId]?.[pessoa.key]?.versiculo).length
  }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">
            {todasPessoas.length} pessoa{todasPessoas.length !== 1 ? 's' : ''} cadastrada
            {todasPessoas.length !== 1 ? 's' : ''}
            {todasPessoas.length > 0 &&
              ` · ${embaixadores.length} embaixador${embaixadores.length !== 1 ? 'es' : ''} · ${conselheiros.length} conselheiro${conselheiros.length !== 1 ? 's' : ''}`}
          </p>
        </div>
        <button
          onClick={() => {
            setShowFormReuniao((value) => !value)
            setNovaReuniaoDias([])
          }}
          className="flex items-center gap-2 px-5 py-2.5 bg-[#171747]! hover:bg-[#0f0f4b]! text-white font-bold rounded-xl transition shadow-lg shadow-[#F6B500]/50 whitespace-nowrap"
        >
          + Nova Reunião
        </button>
      </div>
      ...
      <div className="flex items-center gap-4 mb-5 bg-white/5 rounded-2xl p-4 border border-white/10">
        <button onClick={prevMes} className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition font-bold text-lg leading-none">
          ◀
        </button>
        <span className="flex-1 text-center text-lg font-bold capitalize tracking-wide">
          {MESES[mesAtual]} {anoAtual}
        </span>
        <button onClick={nextMes} className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition font-bold text-lg leading-none">
          ▶
        </button>
      </div>
      ...
      {showFormReuniao && (
        <div className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-md rounded-2xl p-6 border border-[#F6B500]/40 mb-6 shadow-xl shadow-[#F6B500]/5">
          ...
          <p className="text-white/40 text-xs mb-4">
            ...
            {novaReuniaoDias.length > 0 && (
              <span className="ml-2 text-[#F6B500] font-semibold">
                {novaReuniaoDias.length} selecionado{novaReuniaoDias.length > 1 ? 's' : ''}
              </span>
            )}
          </p>
          <div className="grid grid-cols-7 gap-1.5 mb-1">
            {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map((dia) => (
              <div key={dia} className="py-1 text-center text-xs font-bold text-white/30 uppercase tracking-wider">
                {dia}
              </div>
            ))}
          </div>
          <div className="grid grid-cols-7 gap-1.5 mb-5">
            ...
            {Array.from({ length: diasNoMes(mesAtual, anoAtual) }, (_, index) => index + 1).map((dia) => (
              <button
                key={dia}
                type="button"
                onClick={() => toggleDia(dia)}
                className={`py-2 rounded-lg text-sm font-semibold transition ${novaReuniaoDias.includes(dia) ? 'bg-[#F6B500] text-black shadow-lg shadow-[#F6B500]/30 scale-105' : 'bg-white/10 text-white/60 hover:bg-white/20 hover:text-white'}`}
              >
                {dia}
              </button>
            ))}
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-end">
            ...
            <div className="flex gap-2">
              <button
                onClick={() => void handleAddReuniao()}
                disabled={novaReuniaoDias.length === 0}
                className="px-6 py-2.5 bg-[#F6B500] text-white font-bold rounded-lg hover:bg-[#FFD700] transition disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Adicionar
              </button>
              <button
      ...
      {reunioesFiltradas.length > 0 ? (
        <div className="flex gap-2 flex-wrap mb-6">
          {reunioesFiltradas.map((reuniao) => {
            const pCount = presentsCount(reuniao.id)
            const vCount = versiculoCount(reuniao.id)
            const isSelected = reuniaoSelecionada === reuniao.id

            return (
              <div key={reuniao.id} className="flex items-center gap-1">
                <button
                  onClick={() => setReuniaoSelecionada(isSelected ? null : reuniao.id)}
                  className={`px-4 py-2 rounded-xl text-sm font-semibold border transition ${isSelected ? 'bg-[#F6B500] text-black border-[#F6B500] shadow-lg shadow-[#F6B500]/20' : 'bg-white/10 text-white border-white/20 hover:border-[#F6B500]/60 hover:bg-white/15'}`}
                >
                  <span>Dia {reuniao.dia} • {reuniao.horario}</span>
                  {todasPessoas.length > 0 && (
                    <>
                ...
                <button
                  onClick={() => void onDeleteReuniao(reuniao.id)}
      ...
      {reuniaoSelecionada &&
        (() => {
          const reuniao = reunioes.find((item) => item.id === reuniaoSelecionada)
                ...
                <div className="flex flex-wrap gap-6 items-center">
                  <div>
                    ...
                    <p className="text-white font-bold mt-0.5">
                      Dia {reuniao.dia} de {MESES[mesAtual]} • {reuniao.horario}
                    </p>
                  </div>
                  <div className="flex gap-6">
                    <div>
                      <p className="text-xs text-white/40 uppercase tracking-widest font-semibold">Presentes</p>
                      <p className="text-green-400 font-extrabold text-xl mt-0.5">
                        {presentes}
                        <span className="text-white/30 text-sm font-normal">/{total}</span>
                      </p>
                    </div>
                    <div>
                      <p className="text-xs text-white/40 uppercase tracking-widest font-semibold">Versículo</p>
                      <p className="text-[#F6B500] font-extrabold text-xl mt-0.5">
                        {versiculos}
                        <span className="text-white/30 text-sm font-normal">/{total}</span>
                      </p>
                    </div>
              ...
              {total === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {todasPessoas.map((pessoa, index) => {
                        const entry = getEntry(reuniaoSelecionada, pessoa.key)
                        ...
                        return (
                          ...
                          >
                            <td className="p-4 text-white/20 text-xs font-mono">
                            ...
                            <td className="p-4 hidden sm:table-cell">
                              <span
                                className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${pessoa.tipo === 'Embaixador' ? 'bg-[#F6B500]/15 text-[#F6B500]' : 'bg-blue-500/15 text-blue-400'}`}
                              >
                                {pessoa.tipo === 'Embaixador'
                                  ? `👑 ${pessoa.manual}`
                                  : '📋 Conselheiro'}
                              </span>
                            </td>
                            <td className="p-4">
                              <button
                                onClick={() =>
                                  void onTogglePresenca(reuniaoSelecionada, pessoa.key, 'presente')
                                }
                                className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-200 ${entry.presente ? 'bg-green-500 text-white shadow-lg shadow-green-500/25 hover:bg-green-600 scale-105' : 'bg-white/8 text-white/35 hover:bg-white/15 hover:text-white/60 border border-white/10'}`}
                              >
                            ...
                            <td className="p-4">
                              <button
                                onClick={() =>
                                  void onTogglePresenca(reuniaoSelecionada, pessoa.key, 'versiculo')
                                }
                                className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-bold transition-all duration-200 ${entry.versiculo ? 'bg-[#F6B500] text-white shadow-lg shadow-[#F6B500]/25 hover:bg-[#FFD700] scale-105' : 'bg-white/8 text-white/35 hover:bg-white/15 hover:text-white/60 border border-white/10'}`}
                              >
