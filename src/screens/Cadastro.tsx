...
const STEPS = [
  ...
  { num: 3, label: 'Seguranca' },
]

const STEP_TITLES = ['Seus Dados', 'Sua Embaixada', 'Quase la!']
const STEP_SUBTITLES = [
  'Junte-se aos Embaixadores do Rei',
  'Conte-nos sobre sua embaixada',
...
function Cadastro() {
  ...
  function validate(s: number): boolean {
    ...
    if (s === 1) {
      if (!form.username.trim()) e.username = 'Obrigatorio'
      if (!form.nomeConselheiro.trim()) e.nomeConselheiro = 'Obrigatorio'
      if (!form.nomeEmbaixada.trim()) e.nomeEmbaixada = 'Obrigatorio'
    }

    if (s === 2) {
      if (!form.email.trim()) e.email = 'Obrigatorio'
      else if (!/\S+@\S+\.\S+/.test(form.email)) e.email = 'Email invalido'
      if (!form.quantidadePessoas) e.quantidadePessoas = 'Selecione uma opcao'
      if (!form.cidade.trim()) e.cidade = 'Obrigatorio'
      if (!form.estado.trim()) e.estado = 'Obrigatorio'
      if (!form.nomeIgreja.trim()) e.nomeIgreja = 'Obrigatorio'
    }

    if (s === 3) {
      if (!form.senha) e.senha = 'Obrigatorio'
      else if (form.senha.length < 6) e.senha = 'Minimo 6 caracteres'
      if (!form.confirmarSenha) e.confirmarSenha = 'Obrigatorio'
      else if (form.senha !== form.confirmarSenha) e.confirmarSenha = 'As senhas nao coincidem'
    }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (await usernameEmUso()) {
      setErrors((current) => ({ ...current, username: 'Usuario ja esta em uso' }))
      setStep(1)
      setCarregando(false)
      return
    }
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          nomeConselheiro: form.nomeConselheiro.trim(),
          nomeEmbaixada: form.nomeEmbaixada.trim(),
          email: form.email.trim(),
    ...
    setCarregando(false)
    setSucesso('Pra entrar na plataforma confirme seu email antes de entrar.')
  }
    ...
    <div className="login-page">
      ...
      >
        ...
        <div
          ...
          style={{ overflowY: 'auto', paddingTop: 28, paddingBottom: 24 }}
        >
          <div
            style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start', marginBottom: 20 }}
          >
            {STEPS.map((s, i) => (
          ...
          {erroGeral && (
            <p style={{ color: '#fca5a5', fontSize: 13, textAlign: 'center', marginBottom: 12 }}>
              {erroGeral}
            </p>
          )}
              ...
              <div style={boxStyle(28)}>
                ...
                >
                  <option value="" disabled style={{ background: '#0A0A1A' }}>
                    Pessoas na embaixada
                  </option>
                  <option value="0-20" style={{ background: '#0A0A1A' }}>
                    0 a 20 pessoas
                  </option>
                  <option value="21-1000" style={{ background: '#0A0A1A' }}>
                    21 a 1000 pessoas
                  </option>
                </select>
                <i
          ...
          {step === 3 && (
            <form noValidate onSubmit={(e) => void handleSubmit(e)}>
              <div style={boxStyle(24)}>
                <div className="input-box animation" style={{ '--i': 1, '--j': 1, margin: 0, height: '100%' } as React.CSSProperties}>
                  <input type={mostrarSenha ? 'text' : 'password'} required value={form.senha} onChange={(e) => set('senha', e.target.value)} />
                  <label>Senha</label>
                  <i
              ...
              <div style={boxStyle(32)}>
                <div className="input-box animation" style={{ '--i': 2, '--j': 2, margin: 0, height: '100%' } as React.CSSProperties}>
                  <input type={mostrarConfirmar ? 'text' : 'password'} required value={form.confirmarSenha} onChange={(e) => set('confirmarSenha', e.target.value)} />
                  <label>Confirmar Senha</label>
                  <i
                    className={`bx ${mostrarConfirmar ? 'bxs-lock-open' : 'bxs-lock'}`}
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {[
                    ...
                    [
                      'Membros',
                      form.quantidadePessoas === '0-20'
                        ? '0 a 20 pessoas'
                        : form.quantidadePessoas === '21-1000'
                          ? '21 a 1000 pessoas'
                          : '',
                    ],
        ...
        <div className="info-text register">
          <h2 className="animation" style={{ '--i': 0, '--j': 0 } as React.CSSProperties}>
            {step === 1 ? 'Bem-vindo!' : step === 2 ? 'Embaixada' : 'Quase la!'}
          </h2>
      ...
      {sucesso && (
        ...
        >
          <div
            style={{
              background: 'linear-gradient(135deg, #0A0A1A, #13132a)',
              ...
              width: '100%',
              textAlign: 'center',
              boxShadow: '0 20px 60px rgba(246,181,0,0.25)',
            }}
          >
            <div
              style={{
            ...
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 14, marginBottom: 26, lineHeight: 1.5 }}>
              {sucesso}
            </p>
            <button
              type="button"
              ...
              style={{
                width: '100%',
                height: 48,
                background: 'linear-gradient(135deg, #F6B500, #FFD700)',
                border: 'none',
                borderRadius: 12,
                color: '#000',
                fontSize: 15,
